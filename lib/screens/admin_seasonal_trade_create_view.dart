...
class AdminSeasonalTradeCreateView extends StatefulWidget {
  final SeasonalStrategyUserSettings userSettings;

  const AdminSeasonalTradeCreateView({
    super.key, 
    required this.userSettings,
  });

  @override
  State<AdminSeasonalTradeCreateView> createState() => _AdminSeasonalTradeCreateViewState();
}

class _AdminSeasonalTradeCreateViewState extends State<AdminSeasonalTradeCreateView> {
  ...
  Future<void> _save() async {
    ...
    try {
      // 1. Create Trade
      final newTrade = SeasonalTrade(
        symbol: _symbolController.text,
        openDate: _formatDate(_openMonth, _openDay),
        closeDate: _formatDate(_closeMonth, _closeDay),
        direction: _direction,
        verifiedByApi: false, // Default
        symbolExists: false,
        tradeDirectionPossible: false,
      );

      await _apiService.createSeasonalTrade(newTrade);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error creating trade: $e')));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('New Seasonal Trade', style: AppTextStyles.headlineLarge),
        backgroundColor: AppColors.background,
        centerTitle: false,
